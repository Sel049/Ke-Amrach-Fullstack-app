...
// Load environment variables from server/.env (current directory is server when run via npm)
const serverEnvPath = path.join(process.cwd(), '.env');
...
(async () => {
  ...
  // Schema path relative to server directory
  ...
  try {
    ...
    if (existing.length === 0) {
      ...
      await conn.query(
        "INSERT INTO admins (user_id, role, status) VALUES (?, 'superadmin', 'active') ON DUPLICATE KEY UPDATE role=VALUES(role), status=VALUES(status)",
        [userId]
      );
